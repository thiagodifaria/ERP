...
run_webhook_hub_runtime_smoke() {
  ...
  filtered_response="$(curl -fsS "$base_url/api/webhook-hub/events?provider=stripe&event_type=payment.succeeded&status=validated")"
  ...
  if [[ -z "$created_public_id" || "$create_response" != *'"provider":"stripe"'* || "$create_response" != *'"event_type":"payment.succeeded"'* || "$duplicate_response" != *'"code":"webhook_event_conflict"'* || "$duplicate_response" != *'HTTP_STATUS:409'* || "$validate_response" != *'"status":"validated"'* || "$detail_response" != *"\"public_id\":\"$created_public_id\""* || "$detail_response" != *'"external_id":"evt_runtime_001"'* || "$detail_response" != *'"status":"validated"'* || "$filtered_response" != *'"provider":"stripe"'* || "$filtered_response" != *'"status":"validated"'* || "$list_response" != *'"external_id":"evt_runtime_001"'* || "$summary_response" != *'"total":1'* || "$summary_response" != *'"received":0'* || "$summary_response" != *'"stripe":1'* ]]; then
