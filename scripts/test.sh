...
run_webhook_hub_runtime_smoke() {
  ...
  local dlq_create_response
  local dlq_public_id
  local dlq_validate_response
  local dlq_queue_response
  local dlq_process_response
  local dlq_fail_response
  local dlq_requeue_response
  local dlq_reprocess_response
  local dlq_refail_response
  local dead_letter_response
  local dead_letter_list_response
  local transitions_response
  local dlq_transitions_response
  ...
  dlq_create_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"provider":"meta","event_type":"lead.generated","external_id":"evt_runtime_dlq_001","payload_summary":"Lead externo enviado para DLQ no smoke."}' \
    "$base_url/api/webhook-hub/events")"
  dlq_public_id="$(echo "$dlq_create_response" | sed -n 's/.*"public_id":"\([^"]*\)".*/\1/p')"
  dlq_validate_response="$(curl -fsS -X POST "$base_url/api/webhook-hub/events/$dlq_public_id/validate")"
  dlq_queue_response="$(curl -fsS -X POST "$base_url/api/webhook-hub/events/$dlq_public_id/queue")"
  dlq_process_response="$(curl -fsS -X POST "$base_url/api/webhook-hub/events/$dlq_public_id/process")"
  dlq_fail_response="$(curl -fsS -X POST "$base_url/api/webhook-hub/events/$dlq_public_id/fail")"
  dlq_requeue_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"reason":"Retry after provider rate limit."}' \
    "$base_url/api/webhook-hub/events/$dlq_public_id/requeue")"
  dlq_reprocess_response="$(curl -fsS -X POST "$base_url/api/webhook-hub/events/$dlq_public_id/process")"
  dlq_refail_response="$(curl -fsS -X POST "$base_url/api/webhook-hub/events/$dlq_public_id/fail")"
  dead_letter_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"error_code":"provider_timeout","error_message":"Provider timeout on retry path."}' \
    "$base_url/api/webhook-hub/events/$dlq_public_id/dead-letter")"
  transitions_response="$(curl -fsS "$base_url/api/webhook-hub/events/$created_public_id/transitions")"
  dlq_transitions_response="$(curl -fsS "$base_url/api/webhook-hub/events/$dlq_public_id/transitions")"
  ...
  dead_letter_list_response="$(curl -fsS "$base_url/api/webhook-hub/events/dead-letter?provider=meta")"
  ...
  echo "[test] webhook-hub dlq create => $dlq_create_response"
  echo "[test] webhook-hub dlq validate => $dlq_validate_response"
  echo "[test] webhook-hub dlq queue => $dlq_queue_response"
  echo "[test] webhook-hub dlq process => $dlq_process_response"
  echo "[test] webhook-hub dlq fail => $dlq_fail_response"
  echo "[test] webhook-hub dlq requeue => $dlq_requeue_response"
  echo "[test] webhook-hub dlq reprocess => $dlq_reprocess_response"
  echo "[test] webhook-hub dlq refail => $dlq_refail_response"
  echo "[test] webhook-hub dead-letter => $dead_letter_response"
  echo "[test] webhook-hub transitions => $transitions_response"
  echo "[test] webhook-hub dlq transitions => $dlq_transitions_response"
  echo "[test] webhook-hub detail => $detail_response"
  echo "[test] webhook-hub filtered list => $filtered_response"
  echo "[test] webhook-hub dead-letter list => $dead_letter_list_response"
  ...
  if [[ -z "$created_public_id" || -z "$dlq_public_id" || "$create_response" != *'"provider":"stripe"'* || "$create_response" != *'"event_type":"payment.succeeded"'* || "$duplicate_response" != *'"code":"webhook_event_conflict"'* || "$duplicate_response" != *'HTTP_STATUS:409'* || "$validate_response" != *'"status":"validated"'* || "$queue_response" != *'"status":"queued"'* || "$process_response" != *'"status":"processing"'* || "$forward_response" != *'"status":"forwarded"'* || "$dlq_validate_response" != *'"status":"validated"'* || "$dlq_queue_response" != *'"status":"queued"'* || "$dlq_process_response" != *'"status":"processing"'* || "$dlq_fail_response" != *'"status":"failed"'* || "$dlq_requeue_response" != *'"status":"queued"'* || "$dlq_reprocess_response" != *'"status":"processing"'* || "$dlq_refail_response" != *'"status":"failed"'* || "$dead_letter_response" != *'"status":"dead_letter"'* || "$dead_letter_response" != *'"retry_count":1'* || "$dead_letter_response" != *'"last_error_code":"provider_timeout"'* || "$transitions_response" != *'"status":"received"'* || "$transitions_response" != *'"status":"validated"'* || "$transitions_response" != *'"status":"queued"'* || "$transitions_response" != *'"status":"processing"'* || "$transitions_response" != *'"status":"forwarded"'* || "$dlq_transitions_response" != *'"status":"dead_letter"'* || "$detail_response" != *"\"public_id\":\"$created_public_id\""* || "$detail_response" != *'"external_id":"evt_runtime_001"'* || "$detail_response" != *'"status":"forwarded"'* || "$filtered_response" != *'"provider":"stripe"'* || "$filtered_response" != *'"status":"forwarded"'* || "$dead_letter_list_response" != *'"provider":"meta"'* || "$dead_letter_list_response" != *'"status":"dead_letter"'* || "$list_response" != *'"external_id":"evt_runtime_001"'* || "$list_response" != *'"external_id":"evt_runtime_dlq_001"'* || "$summary_response" != *'"total":2'* || "$summary_response" != *'"received":0'* || "$summary_response" != *'"pending_delivery":0'* || "$summary_response" != *'"handled":2'* || "$summary_response" != *'"dead_letter":1'* || "$summary_response" != *'"stripe":1'* || "$summary_response" != *'"meta":1'* || "$summary_response" != *'"forwarded":1'* || "$summary_response" != *'"dead_letter":1'* || "$db_summary" != '2|1|1|1|14' ]]; then
...
run_documents_runtime_smoke() {
  ...
  local upload_session_response
  local upload_session_public_id
  ...
  create_version_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"tenantSlug":"bootstrap-ops","fileName":"bootstrap-proposal-v2.pdf","contentType":"application/pdf","storageKey":"crm/bootstrap-proposal-v2.pdf","storageDriver":"manual","source":"crm","uploadedBy":"documents-smoke","fileSizeBytes":9216,"checksumSha256":"BEEFCAFE1234"}' \
    "$base_url/api/documents/attachments/$created_attachment_public_id/versions")"
  versions_response="$(curl -fsS "$base_url/api/documents/attachments/$created_attachment_public_id/versions?tenantSlug=bootstrap-ops")"
  ...
  upload_session_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d "{\"tenantSlug\":\"bootstrap-ops\",\"ownerType\":\"crm.customer\",\"ownerPublicId\":\"$bootstrap_customer_public_id\",\"fileName\":\"customer-evidence.png\",\"contentType\":\"image/png\",\"storageDriver\":\"local\",\"source\":\"crm\",\"requestedBy\":\"documents-smoke\",\"visibility\":\"restricted\",\"retentionDays\":120,\"expiresInSeconds\":900}" \
    "$base_url/api/documents/upload-sessions")"
  upload_session_public_id="$(echo "$upload_session_response" | sed -n 's/.*"publicId":"\([^"]*\)".*/\1/p')"
  upload_session_detail_response="$(curl -fsS "$base_url/api/documents/upload-sessions/$upload_session_public_id?tenantSlug=bootstrap-ops")"
  upload_session_complete_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"tenantSlug":"bootstrap-ops","uploadedBy":"documents-smoke","fileSizeBytes":10240,"checksumSha256":"AA11BB22CC33DD44"}' \
    "$base_url/api/documents/upload-sessions/$upload_session_public_id/complete")"
  ...
  echo "[test] documents upload session => $upload_session_response"
  echo "[test] documents upload session detail => $upload_session_detail_response"
  echo "[test] documents upload session complete => $upload_session_complete_response"
...
run_crm_runtime_smoke() {
  ...
  pipeline_config_response="$(curl -fsS \
    -X PUT \
    -H "Content-Type: application/json" \
    -d '{"tenantSlug":"bootstrap-ops","name":"Bootstrap Revenue Pipeline","autoScoring":true,"stages":[{"key":"captured","name":"Captured","requiresApproval":false},{"key":"qualified","name":"Qualified","requiresApproval":false},{"key":"proposal","name":"Proposal","requiresApproval":true}],"territoryRules":[{"key":"southeast","name":"Southeast Desk","assignmentMode":"owner-region"}],"approvalPolicies":[{"key":"discount-approval","name":"Discount Approval","approvalScope":"proposal","requiredRole":"manager"}]}' \
    "$base_url/api/crm/pipeline/config")"
  pipeline_config_detail_response="$(curl -fsS "$base_url/api/crm/pipeline/config?tenantSlug=bootstrap-ops")"
  intelligence_summary_response="$(curl -fsS "$base_url/api/crm/leads/intelligence/summary?tenantSlug=bootstrap-ops")"
  echo "[test] crm api pipeline config => $pipeline_config_response"
  echo "[test] crm api pipeline config detail => $pipeline_config_detail_response"
  echo "[test] crm api intelligence summary => $intelligence_summary_response"

  if [[ "$pipeline_config_response" != *'"name":"Bootstrap Revenue Pipeline"'* || "$pipeline_config_detail_response" != *'"autoScoring":true'* || "$pipeline_config_detail_response" != *'"assignmentMode":"owner-region"'* || "$pipeline_config_detail_response" != *'"requiredRole":"manager"'* || "$intelligence_summary_response" != *'"pipelineName":"Bootstrap Revenue Pipeline"'* || "$intelligence_summary_response" != *'"stageCount":3'* || "$intelligence_summary_response" != *'"autoScoring":true'* ]]; then
    echo "[test] CRM intelligence controls did not persist the expected pipeline configuration"
    exit 1
  fi
...
run_finance_runtime_smoke() {
  ...
  activity_response="$(curl -fsS "$base_url/api/finance/activity?tenantSlug=bootstrap-ops")"
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql \
    psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        count(*) || '|' ||
        ...
        (SELECT count(*) FROM finance.activity_events AS event INNER JOIN identity.tenants AS tenant ON tenant.id = event.tenant_id WHERE tenant.slug = 'bootstrap-ops')
  ...
  echo "[test] finance activity => $activity_response"
  ...
  if [[ -z "$created_open_receivable_public_id" || -z "$created_payable_public_id" || -z "$created_cash_account_public_id" \
    ...
    || "$sync_response" != *'"createdReceivables":5'* \
    ...
    || "$receivables_response" != *'"sourceKind":"rental_charge"'* \
    ...
    || "$paid_receivables_response" != *'"amountCents":165000'* \
    ...
    || "$treasury_sync_response" != *'"createdMovements":4'* \
    ...
    || "$cash_movement_summary_response" != *'"total":4'* \
    || "$cash_movement_summary_response" != *'"inflowCents":315000'* \
    || "$cash_movement_summary_response" != *'"outflowCents":42000'* \
    || "$cash_movement_summary_response" != *'"currentBalanceCents":573000'* \
    || "$cash_movement_summary_response" != *'"receivableSettlements":2'* \
    || "$cash_movement_summary_response" != *'"payablePayments":1'* \
    || "$cash_movement_summary_response" != *'"costEntries":1'* \
    || "$treasury_report_response" != *'"currentBalanceCents":573000'* \
    || "$treasury_report_response" != *'"pendingReceivablesCents":125000'* \
    || "$treasury_report_response" != *'"pendingPayablesCents":0'* \
    || "$treasury_report_response" != *'"projectedNetPositionCents":698000'* \
    ...
    || "$operations_report_response" != *'"total":5'* \
    || "$operations_report_response" != *'"paidAmountCents":420000'* \
    ...
    || "$activity_response" != *'"activityType":"receivable_synced"'* \
    || "$activity_response" != *'"activityType":"receivable_settled"'* \
    || "$activity_response" != *'"activityType":"payable_created"'* \
    || "$activity_response" != *'"activityType":"payable_status_changed"'* \
    || "$activity_response" != *'"activityType":"cost_created"'* \
    || "$activity_response" != *'"activityType":"treasury_synced"'* \
    || "$activity_response" != *'"activityType":"period_closed"'* \
    || "$db_summary" != '3|1|0|1|1|105000|105000|5|1|3|1|0|420000|1|0|0|1|8400|1|0|1|0|19000|1|23000|1|1|1|4|315000|42000|7' ]]; then
...
run_workflow_runtime_smoke() {
  ...
  if [[ -z "$created_public_id" || -z "$cancel_public_id" || -z "$fail_public_id" || "$create_response" != *'"tenantSlug":"bootstrap-ops"'* || "$create_response" != *'"workflowDefinitionVersionNumber":1'* || "$start_response" != *'"status":"running"'* || "$advance_task_response" != *'"currentActionIndex":1'* || ( "$waiting_response" != *'"waitingUntil":"'* && "$waiting_response" != *'"code":"workflow_runtime_execution_waiting"'* ) || "$waiting_conflict_response" != *'"code":"workflow_runtime_execution_waiting"'* || "$waiting_conflict_response" != *'HTTP_STATUS:409'* || "$release_response" != *'"currentActionIndex":2'* || "$release_response" != *'"waitingUntil":null'* || "$complete_response" != *'"status":"completed"'* || "$complete_response" != *'"completedAt":"'*
...
run_billing_runtime_smoke() {
  ...
  created_invoice_public_id="$(echo "$create_invoice_response" | sed -n 's/.*"publicId":"\([^"]*\)".*/\1/p')"
  invoice_detail_response="$(curl -fsS "$base_url/api/billing/invoices/$created_invoice_public_id?tenantSlug=bootstrap-ops")"
  ...
  billing_events_response="$(curl -fsS "$base_url/api/billing/events?tenantSlug=bootstrap-ops&invoicePublicId=$created_invoice_public_id")"
  ...
  echo "[test] billing invoice detail => $invoice_detail_response"
  ...
  echo "[test] billing events => $billing_events_response"
...
run_engagement_runtime_smoke() {
  ...
  local inbound_lead_response
  local workflow_dispatch_response
  local dispatch_touchpoint_public_id
  local dispatch_delivery_public_id
  local provider_delivery_callback_response
  local provider_response_callback_response
  local provider_event_detail_response
  local callback_provider_event_public_id
  local provider_events_response
  local provider_events_filtered_response
  local provider_events_summary_response
  ...
  workflow_dispatch_response="$(curl -fsS \
    -X POST \
    -H "Content-Type: application/json" \
    -d "{\"tenantSlug\":\"bootstrap-ops\",\"campaignPublicId\":\"$created_campaign_public_id\",\"templatePublicId\":\"$created_template_public_id\",\"leadPublicId\":\"$CRM_RUNTIME_LEAD_PUBLIC_ID\",\"contactValue\":\"runtime.lead.prime@example.com\",\"provider\":\"resend\",\"workflowRunPublicId\":\"00000000-0000-0000-0000-000000000451\",\"providerMessageId\":\"msg-runtime-002\",\"createdBy\":\"engagement-smoke\",\"notes\":\"Dispatch criado pelo smoke de workflow.\"}" \
    "$base_url/api/engagement/workflow-dispatches")"
  dispatch_touchpoint_public_id="$(echo "$workflow_dispatch_response" | sed -n 's/.*"touchpoint":{[^}]*"publicId":"\([^"]*\)".*/\1/p')"
  dispatch_delivery_public_id="$(echo "$workflow_dispatch_response" | sed -n 's/.*"delivery":{[^}]*"publicId":"\([^"]*\)".*/\1/p')"
  ...
  provider_events_filtered_response="$(curl -fsS "$base_url/api/engagement/provider-events?tenantSlug=bootstrap-ops&provider=resend&direction=inbound")"
  provider_events_summary_response="$(curl -fsS "$base_url/api/engagement/provider-events/summary?tenantSlug=bootstrap-ops")"
  ...
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql \
    psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        ...
        (SELECT count(*) FROM engagement.provider_events AS event INNER JOIN identity.tenants AS tenant ON tenant.id = event.tenant_id WHERE tenant.slug = 'bootstrap-ops');
    ")"
  ...
  echo "[test] engagement inbound lead => $inbound_lead_response"
  echo "[test] engagement workflow dispatch => $workflow_dispatch_response"
  echo "[test] engagement provider delivered callback => $provider_delivery_callback_response"
  echo "[test] engagement provider responded callback => $provider_response_callback_response"
  echo "[test] engagement provider event detail => $provider_event_detail_response"
  echo "[test] engagement provider events => $provider_events_response"
  echo "[test] engagement provider events filtered => $provider_events_filtered_response"
  echo "[test] engagement provider events summary => $provider_events_summary_response"
  ...
  if [[ -z "$created_campaign_public_id" || -z "$created_template_public_id" || -z "$created_touchpoint_public_id" || -z "$created_delivery_public_id" || -z "$dispatch_touchpoint_public_id" || -z "$dispatch_delivery_public_id" || -z "$callback_provider_event_public_id" || "$update_campaign_status_response" != *'"status":"active"'* || "$create_template_response" != *'"provider":"resend"'* || "$update_template_status_response" != *'"status":"active"'* || "$template_detail_response" != *'"key":"runtime-reactivation-email"'* || "$create_touchpoint_response" != *"\"leadPublicId\":\"$CRM_RUNTIME_LEAD_PUBLIC_ID\""* || "$create_touchpoint_response" != *'"status":"queued"'* || "$create_delivery_response" != *'"status":"sent"'* || "$create_delivery_response" != *'"provider":"resend"'* || "$create_delivery_response" != *"\"templatePublicId\":\"$created_template_public_id\""* || "$deliveries_response" != *"\"publicId\":\"$created_delivery_public_id\""* || "$update_delivery_status_response" != *'"status":"delivered"'* || "$update_touchpoint_status_response" != *'"status":"converted"'* || "$update_touchpoint_status_response" != *'"lastWorkflowRunPublicId":"00000000-0000-0000-0000-000000000390"'* || "$inbound_lead_response" != *'"source":"meta_ads"'* || "$inbound_lead_response" != *'"eventType":"lead.ingested"'* || "$workflow_dispatch_response" != *'"eventType":"workflow.dispatched"'* || "$provider_delivery_callback_response" != *'"status":"processed"'* || "$provider_response_callback_response" != *'"status":"processed"'* || "$provider_event_detail_response" != *"\"publicId\":\"$callback_provider_event_public_id\""* || "$provider_event_detail_response" != *'"eventType":"delivery.responded"'* || "$provider_event_detail_response" != *'"responseSummary":"Provider response linked to touchpoint."'* || "$provider_events_response" != *'"eventType":"workflow.dispatched"'* || "$provider_events_response" != *'"eventType":"delivery.delivered"'* || "$provider_events_response" != *'"eventType":"delivery.responded"'* || "$provider_events_filtered_response" != *'"eventType":"delivery.delivered"'* || "$provider_events_filtered_response" != *'"eventType":"delivery.responded"'* || "$provider_events_summary_response" != *'"total":3'* || "$provider_events_summary_response" != *'"resend":3'* || "$provider_events_summary_response" != *'"inbound":2'* || "$provider_events_summary_response" != *'"outbound":1'* || "$touchpoint_detail_response" != *'"channel":"email"'* || "$filtered_touchpoints_response" != *"\"publicId\":\"$created_touchpoint_public_id\""* || "$summary_response" != *'"touchpoints":3'* || "$summary_response" != *'"workflowDispatched":3'* || "$summary_response" != *'"responded":2'* || "$summary_response" != *'"converted":1'* || "$summary_response" != *'"email":2'* || "$delivery_summary_response" != *'"templates":2'* || "$delivery_summary_response" != *'"deliveries":3'* || "$delivery_summary_response" != *'"delivered":3'* || "$delivery_summary_response" != *'"resend":2'* || "$delivery_summary_response" != *'"manual":1'* || "$db_summary" != "3|2|3|3|2|1|2|2|1|3|0|3|3" ]]; then
...
run_analytics_runtime_smoke() {
  ...
  local platform_reliability_response
  local hardening_review_response
  local core_operations_response
  local relationship_intelligence_response
  local compliance_control_response
  local go_live_control_response
  local saas_control_response
  local contract_governance_response
  ...
  platform_reliability_response="$(curl -fsS "$base_url/api/analytics/reports/platform-reliability?tenant_slug=bootstrap-ops")"
  hardening_review_response="$(curl -fsS "$base_url/api/analytics/reports/hardening-review?tenant_slug=bootstrap-ops")"
  core_operations_response="$(curl -fsS "$base_url/api/analytics/reports/core-operations?tenant_slug=bootstrap-ops")"
  relationship_intelligence_response="$(curl -fsS "$base_url/api/analytics/reports/relationship-intelligence?tenant_slug=bootstrap-ops")"
  compliance_control_response="$(curl -fsS "$base_url/api/analytics/reports/compliance-control?tenant_slug=bootstrap-ops")"
  go_live_control_response="$(curl -fsS "$base_url/api/analytics/reports/go-live-control?tenant_slug=bootstrap-ops")"
  saas_control_response="$(curl -fsS "$base_url/api/analytics/reports/saas-control?tenant_slug=bootstrap-ops")"
  contract_governance_response="$(curl -fsS "$base_url/api/analytics/reports/contract-governance")"
  ...
  echo "[test] analytics platform reliability => $platform_reliability_response"
  echo "[test] analytics hardening review => $hardening_review_response"
  echo "[test] analytics core operations => $core_operations_response"
  echo "[test] analytics relationship intelligence => $relationship_intelligence_response"
  echo "[test] analytics compliance control => $compliance_control_response"
  echo "[test] analytics go-live control => $go_live_control_response"
  echo "[test] analytics saas control => $saas_control_response"
  echo "[test] analytics contract governance => $contract_governance_response"
  echo "[test] analytics cost estimator => $cost_estimator_response"
  echo "[test] analytics load benchmark => $load_benchmark_response"

  if [[ "$health_details_response" != *'"name":"report-engine","status":"ready"'* || "$health_details_response" != *'"name":"postgresql","status":"ready"'* || "$health_details_response" != *'"name":"forecast-model","status":"ready"'* || "$health_details_response" != *'"name":"simulation-catalog","status":"ready"'* || "$pipeline_summary_response" != *'"tenantSlug":"bootstrap-ops"'* || "$pipeline_summary_response" != *'"dataSource":"postgresql"'* || "$pipeline_summary_response" != *'"leadsCaptured":2'* || "$pipeline_summary_response" != *'"conversions":3'* || "$pipeline_summary_response" != *'"manual":1'* || "$pipeline_summary_response" != *'"instagram":1'* || "$pipeline_summary_response" != *'"runningAutomations":2'* || "$service_pulse_response" != *'"tenantSlug":"bootstrap-ops"'* || "$service_pulse_response" != *'"dataSource":"postgresql"'* || "$service_pulse_response" != *'"totalLeads":2'* || "$service_pulse_response" != *'"salesTotal":3'* || "$service_pulse_response" != *'"bookedRevenueCents":230000'* || "$service_pulse_response" != *'"receivablesOpen":1'* || "$service_pulse_response" != *'"receivablesPaid":3'* || "$service_pulse_response" != *'"cashAccounts":1'* || "$service_pulse_response" != *'"currentBalanceCents":573000'* || "$service_pulse_response" != *'"activeSubscriptions":1'* || "$service_pulse_response" != *'"invoicesPaid":1'* || "$service_pulse_response" != *'"failedAttempts":2'* || "$service_pulse_response" != *'"attachmentsTotal":6'* || "$service_pulse_response" != *'"activeAttachments":5'* || "$service_pulse_response" != *'"archivedAttachments":1'* || "$service_pulse_response" != *'"pendingUploadSessions":0'* || "$service_pulse_response" != *'"completedUploadSessions":1'* || "$service_pulse_response" != *'"contractsTotal":1'* || "$service_pulse_response" != *'"activeContracts":0'* || "$service_pulse_response" != *'"scheduledCharges":1'* || "$service_pulse_response" != *'"paidCharges":1'* || "$service_pulse_response" != *'"cancelledCharges":1'* || "$service_pulse_response" != *'"activeDefinitions":1'* || "$service_pulse_response" != *'"runsRunning":2'* || "$service_pulse_response" != *'"runsCompleted":2'* || "$service_pulse_response" != *'"runsFailed":1'* || "$service_pulse_response" != *'"runsCancelled":1'* || "$service_pulse_response" != *'"totalExecutions":3'* || "$service_pulse_response" != *'"completed":3'* || "$service_pulse_response" != *'"failed":0'* || "$service_pulse_response" != *'"forwarded":1'* || "$sales_journey_response" != *'"tenantSlug":"bootstrap-ops"'* || "$sales_journey_response" != *'"dataSource":"postgresql"'* || "$sales_journey_response" != *'"leadsCaptured":2'* || "$sales_journey_response" != *'"leadsWithOpportunity":2'* || "$sales_journey_response" != *'"opportunitiesWithProposal":3'* || "$sales_journey_response" != *'"proposalsConverted":3'* || "$sales_journey_response" != *'"salesWon":3'* || "$sales_journey_response" != *'"leadToSaleConversionRate":1.5'* || "$sales_journey_response" != *'"totalAmountCents":308000'* || "$sales_journey_response" != *'"won":3'* || "$sales_journey_response" != *'"accepted":3'* || "$sales_journey_response" != *'"bookedRevenueCents":230000'* || "$sales_journey_response" != *'"active":1'* || "$sales_journey_response" != *'"invoiced":1'* || "$sales_journey_response" != *'"controlRuns":1'* || "$sales_journey_response" != *'"runtimeCompleted":1'* || "$tenant_360_response" != *'"tenantSlug":"bootstrap-ops"'* || "$tenant_360_response" != *'"dataSource":"postgresql"'* || "$tenant_360_response" != *'"companies":1'* || "$tenant_360_response" != *'"users":1'* || "$tenant_360_response" != *'"teams":1'* || "$tenant_360_response" != *'"roles":5'* || "$tenant_360_response" != *'"assignedLeads":2'* || "$tenant_360_response" != *'"leadNotes":2'* || "$tenant_360_response" != *'"opportunities":3'* || "$tenant_360_response" != *'"proposals":3'* || "$tenant_360_response" != *'"sales":3'* || "$tenant_360_response" != *'"bookedRevenueCents":230000'* || "$tenant_360_response" != *'"attachments":6'* || "$tenant_360_response" != *'"activeAttachments":5'* || "$tenant_360_response" != *'"archivedAttachments":1'* || "$tenant_360_response" != *'"uploadSessions":1'* || "$tenant_360_response" != *'"completedUploadSessions":1'* || "$tenant_360_response" != *'"restrictedAttachments":2'* || "$tenant_360_response" != *'"contracts":1'* || "$tenant_360_response" != *'"activeContracts":0'* || "$tenant_360_response" != *'"scheduledCharges":1'* || "$tenant_360_response" != *'"paidCharges":1'* || "$tenant_360_response" != *'"attachments":1'* || "$tenant_360_response" != *'"outstandingAmountCents":165000'* || "$tenant_360_response" != *'"receivables":5'* || "$tenant_360_response" != *'"paidReceivables":3'* || "$tenant_360_response" != *'"commissions":1'* || "$tenant_360_response" != *'"payables":1'* || "$tenant_360_response" != *'"cashAccounts":1'* || "$tenant_360_response" != *'"currentBalanceCents":573000'* || "$tenant_360_response" != *'"plans":1'* || "$tenant_360_response" != *'"activeSubscriptions":1'* || "$tenant_360_response" != *'"paidInvoices":1'* || "$tenant_360_response" != *'"paymentAttempts":3'* || "$tenant_360_response" != *'"failedAttempts":2'* || "$tenant_360_response" != *'"workflowRuns":6'* || "$tenant_360_response" != *'"workflowRunEvents":10'* || "$tenant_360_response" != *'"runtimeExecutions":3'* || "$tenant_360_response" != *'"runtimeCompleted":3'* || "$tenant_360_response" != *'"runtimeFailed":0'* || "$automation_board_response" != *'"tenantSlug":"bootstrap-ops"'* || "$automation_board_response" != *'"dataSource":"postgresql"'* || "$automation_board_response" != *'"definitionsTotal":2'* || "$automation_board_response" != *'"definitionsActive":1'* || "$automation_board_response" != *'"definitionsDraft":1'* || "$automation_board_response" != *'"publishedVersions":3'* || "$automation_board_response" != *'"runsTotal":6'* || "$automation_board_response" != *'"runningRuns":2'* || "$automation_board_response" != *'"recordedEvents":10'* || "$automation_board_response" != *'"workflowDefinitionKey":"lead-follow-up"'* || "$automation_board_response" != *'"executionsTotal":3'* || "$automation_board_response" != *'"completedExecutions":3'* || "$automation_board_response" != *'"failedExecutions":0'* || "$automation_board_response" != *'"retriesTotal":1'* || "$automation_board_response" != *'"recordedTransitions":12'* || "$automation_board_response" != *'"forwarded":1'* || "$workflow_definition_health_response" != *'"tenantSlug":"bootstrap-ops"'* || "$workflow_definition_health_response" != *'"dataSource":"postgresql"'* || "$workflow_definition_health_response" != *'"definitionsTotal":2'* || "$workflow_definition_health_response" != *'"stable":1'* || "$workflow_definition_health_response" != *'"attention":1'* || "$workflow_definition_health_response" != *'"critical":0'* || "$workflow_definition_health_response" != *'"workflowDefinitionKey":"lead-follow-up"'* || "$workflow_definition_health_response" != *'"health":"stable"'* || "$workflow_definition_health_response" != *'"workflowDefinitionKey":"runtime-flow"'* || "$workflow_definition_health_response" != *'"health":"attention"'* || "$workflow_definition_health_response" != *'"definition-not-active"'* || "$delivery_reliability_response" != *'"provider":"stripe"'* || "$delivery_reliability_response" != *'"dataSource":"postgresql"'* || "$delivery_reliability_response" != *'"totalEvents":2'* || "$delivery_reliability_response" != *'"handledEvents":1'* || "$delivery_reliability_response" != *'"avgTransitionsPerEvent":3.0'* || "$delivery_reliability_response" != *'"received":1'* || "$delivery_reliability_response" != *'"validated":0'* || "$delivery_reliability_response" != *'"queued":0'* || "$delivery_reliability_response" != *'"processing":0'* || "$delivery_reliability_response" != *'"forwarded":1'* || "$document_governance_response" != *'"tenantSlug":"bootstrap-ops"'* || "$document_governance_response" != *'"dataSource":"postgresql"'* || "$document_governance_response" != *'"attachmentsTotal":6'* || "$document_governance_response" != *'"activeAttachments":5'* || "$document_governance_response" != *'"archivedAttachments":1'* || "$document_governance_response" != *'"retentionLongTerm":4'* || "$document_governance_response" != *'"internal":4'* || "$document_governance_response" != *'"restricted":2'* || "$document_governance_response" != *'"public":0'* || "$document_governance_response" != *'"manual":4'* || "$document_governance_response" != *'"external":2'* || "$document_governance_response" != *'"local":1'* || "$document_governance_response" != *'"r2":1'* || "$document_governance_response" != *'"sessionsTotal":1'* || "$document_governance_response" != *'"pending":0'* || "$document_governance_response" != *'"completed":1'* || "$document_governance_response" != *'"expired":0'* || "$document_governance_response" != *'"crm.customer":3'* || "$document_governance_response" != *'"crm.lead":2'* || "$document_governance_response" != *'"rentals.contract":1'* || "$revenue_operations_response" != *'"tenantSlug":"bootstrap-ops"'* || "$revenue_operations_response" != *'"dataSource":"postgresql"'* || "$revenue_operations_response" != *'"bookedRevenueCents":230000'* || "$revenue_operations_response" != *'"total":3'* || "$revenue_operations_response" != *'"openAmountCents":125000'* || "$revenue_operations_response" != *'"paidAmountCents":105000'* || "$revenue_operations_response" != *'"overdueAmountCents":0'* || "$revenue_operations_response" != *'"overdueCount":0'* || "$revenue_operations_response" != *'"invoiceCoverageRate":0.6667'* || "$revenue_operations_response" != *'"collectionRate":0.4565'* || "$revenue_operations_response" != *'"averageTicketCents":76666'* || "$revenue_operations_response" != *'"invoicesDueSoon":0'* || "$finance_control_response" != *'"tenantSlug":"bootstrap-ops"'* || "$finance_control_response" != *'"dataSource":"postgresql"'* || "$finance_control_response" != *'"accountsTotal":1'* || "$finance_control_response" != *'"activeAccounts":1'* || "$finance_control_response" != *'"movementsTotal":4'* || "$finance_control_response" != *'"currentBalanceCents":573000'* || "$finance_control_response" != *'"inflowCents":315000'* || "$finance_control_response" != *'"outflowCents":42000'* || "$finance_control_response" != *'"paidAmountCents":420000'* || "$finance_control_response" != *'"dueSoonCount":0'* || "$finance_control_response" != *'"plansTotal":1'* || "$finance_control_response" != *'"activeSubscriptions":1'* || "$finance_control_response" != *'"invoicesPaid":1'* || "$finance_control_response" != *'"failedAttempts":2'* || "$finance_control_response" != *'"succeededAttempts":1'* || "$finance_control_response" != *'"monthlyRecurringRevenueCents":4900'* || "$finance_control_response" != *'"releasedCommissionsCents":8400'* || "$finance_control_response" != *'"operatingCostCents":23000'* || "$finance_control_response" != *'"paidPayablesCents":19000'* || "$finance_control_response" != *'"netOperationalMarginCents":369600'* || "$finance_control_response" != *'"netTreasuryMovementCents":273000'* || "$finance_control_response" != *'"periodClosures":1'* || "$finance_control_response" != *'"subscriptionEvents":7'* || "$finance_control_response" != *'"cashMovements":4'* || "$finance_control_response" != *'"failedPaymentAttempts":2'* || "$rental_operations_response" != *'"tenantSlug":"bootstrap-ops"'* || "$rental_operations_response" != *'"dataSource":"postgresql"'* || "$rental_operations_response" != *'"total":1'* || "$rental_operations_response" != *'"active":0'* || "$rental_operations_response" != *'"terminated":1'* || "$rental_operations_response" != *'"scheduled":1'* || "$rental_operations_response" != *'"paid":1'* || "$rental_operations_response" != *'"cancelled":1'* || "$rental_operations_response" != *'"outstandingAmountCents":165000'* || "$rental_operations_response" != *'"collectedAmountCents":150000'* || "$rental_operations_response" != *'"cancelledAmountCents":165000'* || "$rental_operations_response" != *'"dueSoon":0'* || "$rental_operations_response" != *'"overdue":0'* || "$rental_operations_response" != *'"adjustments":1'* || "$rental_operations_response" != *'"historyEvents":4'* || "$rental_operations_response" != *'"pendingOutbox":4'* || "$rental_operations_response" != *'"attachments":1'* || "$cost_estimator_response" != *'"tenantSlug":"bootstrap-ops"'* || "$cost_estimator_response" != *'"dataSource":"postgresql"'* || "$cost_estimator_response" != *'"estimatedMonthlyCostCents":34992'* || "$cost_estimator_response" != *'"storageProjectedMb":2096'* || "$cost_estimator_response" != *'"risk":"attention"'* || "$cost_estimator_response" != *'"warm-object-storage"'* || "$load_benchmark_response" != *'"tenantSlug":"bootstrap-ops"'* || "$load_benchmark_response" != *'"dataSource":"postgresql"'* || "$load_benchmark_response" != *'"totalRuns":1'* || "$load_benchmark_response" != *'"stable":1'* || "$load_benchmark_response" != *'"avgLatencyMs":56'* || "$load_benchmark_response" != *'"p95LatencyMs":103'* || "$load_benchmark_response" != *'"throughputRps":62.22'* || "$load_benchmark_response" != *'"status":"stable"'* ]]; then
  ...
  if [[ "$core_operations_response" != *'"tenantSlug":"bootstrap-ops"'* || "$core_operations_response" != *'"catalogItems":2'* || "$core_operations_response" != *'"suppliers":1'* || "$core_operations_response" != *'"supportCases":1'* || "$core_operations_response" != *'"notifications":1'* || "$relationship_intelligence_response" != *'"tenantSlug":"bootstrap-ops"'* || "$relationship_intelligence_response" != *'"importsReady":true'* || "$relationship_intelligence_response" != *'"exportsReady":true'* || "$relationship_intelligence_response" != *'"territoryRules":1'* || "$relationship_intelligence_response" != *'"approvalPolicies":1'* || "$compliance_control_response" != *'"tenantSlug":"bootstrap-ops"'* || "$compliance_control_response" != *'"documents":1'* || "$compliance_control_response" != *'"events":3'* || "$compliance_control_response" != *'"requests":1'* || "$compliance_control_response" != *'"granted":1'* || "$go_live_control_response" != *'"tenantSlug":"bootstrap-ops"'* || "$go_live_control_response" != *'"completed":1'* || "$go_live_control_response" != *'"trackedMetrics":1'* || "$go_live_control_response" != *'"releaseControls"'* || "$go_live_control_response" != *'"acceptanceReady":true'* || "$go_live_control_response" != *'"rollbackReady":true'* || "$saas_control_response" != *'"tenantSlug":"bootstrap-ops"'* || "$saas_control_response" != *'"trackedMetrics":1'* || "$saas_control_response" != *'"completed":1'* || "$contract_governance_response" != *'"apiPortalReady":true'* || "$contract_governance_response" != *'"schemaRegistryReady":true'* || "$contract_governance_response" != *'"cursorPaginationReady":true'* ]]; then
    echo "[test] analytics expansion and go-live reports did not expose the expected payload"
    exit 1
  fi
}
...
run_simulation_runtime_smoke() {
  ...
  if [[ "$health_details_response" != *'"name":"scenario-engine","status":"ready"'* || "$health_details_response" != *'"name":"postgresql","status":"ready"'* || "$health_details_response" != *'"name":"cost-model","status":"ready"'* || "$catalog_response" != *'"service":"simulation"'* || "$catalog_response" != *'"key":"operational-load"'* || "$catalog_response" != *'"key":"load-benchmark"'* || -z "$scenario_public_id" || "$create_scenario_response" != *'"tenantSlug":"bootstrap-ops"'* || "$create_scenario_response" != *'"leadsProjected":4'* || "$create_scenario_response" != *'"workflowRunsProjected":14'* || "$create_scenario_response" != *'"storageProjectedMb":2096'* || "$create_scenario_response" != *'"estimatedMonthlyCostCents":34992'* || "$create_scenario_response" != *'"risk":"attention"'* || "$scenario_list_response" != *"\"publicId\":\"$scenario_public_id\""* || "$scenario_detail_response" != *"\"publicId\":\"$scenario_public_id\""* || "$create_benchmark_response" != *'"tenantSlug":"bootstrap-ops"'* || "$create_benchmark_response" != *'"sampleSize":120'* || "$create_benchmark_response" != *'"avgLatencyMs":56'* || "$create_benchmark_response" != *'"p95LatencyMs":103'* || "$create_benchmark_response" != *'"throughputRps":62.22'* || "$create_benchmark_response" != *'"status":"stable"'* || "$benchmark_list_response" != *'"publicId"'* || "$benchmark_list_response" != *'"status":"stable"'* || "$simulation_db_summary" != '1|1' ]]; then
    echo "[test] simulation runtime foundation did not return the expected payload"
    exit 1
  fi
}

run_catalog_runtime_smoke() {
  ...
  "${COMPOSE_CMD[@]}" up -d --build catalog
  wait_for_http_ready "$base_url/health/ready"
  ...
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        tenant.slug || '|' ||
        (SELECT count(*) FROM catalog.categories AS category WHERE category.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM catalog.items AS item WHERE item.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM catalog.items AS item WHERE item.tenant_id = tenant.id AND item.item_type = 'service') || '|' ||
        (SELECT count(*) FROM catalog.items AS item WHERE item.tenant_id = tenant.id AND item.active = false)
      FROM identity.tenants AS tenant
      WHERE tenant.slug = 'bootstrap-ops'
      LIMIT 1;
    ")"
...
run_support_runtime_smoke() {
  local base_url="http://localhost:${SUPPORT_HTTP_PORT:-8099}"
  local capabilities_response
  local queue_response
  local list_response
  local create_response
  local case_public_id
  local status_response
  ...
  "${COMPOSE_CMD[@]}" up -d --build support
  wait_for_http_ready "$base_url/health/ready"

  capabilities_response="$(curl -fsS "$base_url/api/support/capabilities")"
  queue_response="$(curl -fsS -X PUT -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","name":"Backoffice","slaTargetHours":12,"active":true}' "$base_url/api/support/queues/backoffice")"
  create_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","subject":"Customer cannot generate invoice.","queueKey":"backoffice","priority":"high","ownerUserId":"owner-user","sourceKind":"crm","entityKind":"crm.customer","entityPublicId":"customer-001"}' "$base_url/api/support/cases")"
  case_public_id="$(printf '%s' "$create_response" | extract_json_field "publicId")"
  status_response="$(curl -fsS -X PATCH -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","status":"in_progress","summary":"Assigned to finance support."}' "$base_url/api/support/cases/$case_public_id/status")"
  comment_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","message":"Waiting payment gateway callback evidence."}' "$base_url/api/support/cases/$case_public_id/comments")"
  detail_response="$(curl -fsS "$base_url/api/support/cases/$case_public_id?tenant_slug=bootstrap-ops")"
  list_response="$(curl -fsS "$base_url/api/support/cases?tenant_slug=bootstrap-ops&limit=10")"
  summary_response="$(curl -fsS "$base_url/api/support/cases/summary?tenant_slug=bootstrap-ops")"
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        tenant.slug || '|' ||
        (SELECT count(*) FROM support.queues AS queue WHERE queue.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM support.cases AS support_case WHERE support_case.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM support.cases AS support_case WHERE support_case.tenant_id = tenant.id AND support_case.status = 'in_progress') || '|' ||
        (SELECT count(*) FROM support.case_events AS event JOIN support.cases AS support_case ON support_case.id = event.case_id WHERE support_case.tenant_id = tenant.id)
      FROM identity.tenants AS tenant
      WHERE tenant.slug = 'bootstrap-ops'
      LIMIT 1;
    ")"

  echo "[test] support capabilities => $capabilities_response"
  echo "[test] support queue => $queue_response"
  echo "[test] support create => $create_response"
  echo "[test] support status => $status_response"
  echo "[test] support comment => $comment_response"
  echo "[test] support detail => $detail_response"
  echo "[test] support list => $list_response"
  echo "[test] support summary => $summary_response"
  echo "[test] support db summary => $db_summary"

  if [[ "$capabilities_response" != *'"key":"support.sla"'* || "$queue_response" != *'"queueKey":"backoffice"'* || -z "$case_public_id" || "$create_response" != *'"priority":"high"'* || "$status_response" != *'"status":"in_progress"'* || "$comment_response" != *'"eventType":"comment"'* || "$detail_response" != *"\"publicId\":\"$case_public_id\""* || "$list_response" != *'"pageInfo"'* || "$summary_response" != *'"total":1'* || "$summary_response" != *'"high":1'* || "$db_summary" != 'bootstrap-ops|1|1|1|3' ]]; then
    echo "[test] support runtime state did not expose the expected operations cycle"
    exit 1
  fi
}

run_supplier_runtime_smoke() {
  local base_url="http://localhost:${SUPPLIER_HTTP_PORT:-8100}"
  local capabilities_response
  local category_response
  local create_response
  local supplier_public_id
  ...
  "${COMPOSE_CMD[@]}" up -d --build supplier
  wait_for_http_ready "$base_url/health/ready"

  capabilities_response="$(curl -fsS "$base_url/api/supplier/capabilities")"
  category_response="$(curl -fsS -X PUT -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","name":"Software","active":true}' "$base_url/api/supplier/categories/software")"
  create_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","companyName":"ERP Providers Ltda","tradeName":"ERP Providers","taxId":"12.345.678/0001-90","categoryKey":"software","status":"active","payableTermDays":15,"bankName":"Banco Example","pixKey":"financeiro@erp.test","contactEmail":"financeiro@erp.test"}' "$base_url/api/supplier/suppliers")"
  supplier_public_id="$(printf '%s' "$create_response" | extract_json_field "publicId")"
  patch_response="$(curl -fsS -X PATCH -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","status":"watchlist","payableTermDays":30}' "$base_url/api/supplier/suppliers/$supplier_public_id")"
  detail_response="$(curl -fsS "$base_url/api/supplier/suppliers/$supplier_public_id?tenant_slug=bootstrap-ops")"
  list_response="$(curl -fsS "$base_url/api/supplier/suppliers?tenant_slug=bootstrap-ops")"
  summary_response="$(curl -fsS "$base_url/api/supplier/suppliers/summary?tenant_slug=bootstrap-ops")"
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        tenant.slug || '|' ||
        (SELECT count(*) FROM supplier.categories AS category WHERE category.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM supplier.suppliers AS supplier WHERE supplier.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM supplier.suppliers AS supplier WHERE supplier.tenant_id = tenant.id AND supplier.status = 'watchlist')
      FROM identity.tenants AS tenant
      WHERE tenant.slug = 'bootstrap-ops'
      LIMIT 1;
    ")"

  echo "[test] supplier capabilities => $capabilities_response"
  echo "[test] supplier category => $category_response"
  echo "[test] supplier create => $create_response"
  echo "[test] supplier patch => $patch_response"
  echo "[test] supplier detail => $detail_response"
  echo "[test] supplier list => $list_response"
  echo "[test] supplier summary => $summary_response"
  echo "[test] supplier db summary => $db_summary"

  if [[ "$capabilities_response" != *'"key":"supplier.directory"'* || "$category_response" != *'"categoryKey":"software"'* || -z "$supplier_public_id" || "$create_response" != *'"companyName":"ERP Providers Ltda"'* || "$patch_response" != *'"status":"watchlist"'* || "$detail_response" != *"\"publicId\":\"$supplier_public_id\""* || "$list_response" != *'"categoryKey":"software"'* || "$summary_response" != *'"suppliersTotal":1'* || "$db_summary" != 'bootstrap-ops|1|1|1' ]]; then
    echo "[test] supplier runtime state did not expose the expected directory cycle"
    exit 1
  fi
}

run_notification_runtime_smoke() {
  local base_url="http://localhost:${NOTIFICATION_HTTP_PORT:-8101}"
  local capabilities_response
  local preference_response
  local create_response
  local notification_public_id
  local transition_response
  local center_response
  local summary_response
  local db_summary

  "${COMPOSE_CMD[@]}" up -d --build notification
  wait_for_http_ready "$base_url/health/ready"

  capabilities_response="$(curl -fsS "$base_url/api/notification/capabilities")"
  preference_response="$(curl -fsS -X PUT -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","inAppEnabled":true,"emailEnabled":true,"quietHours":{"from":"23:00","to":"06:00"}}' "$base_url/api/notification/preferences/user-001")"
  create_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","userPublicId":"user-001","title":"Payment recovery needs review","body":"A billing recovery case escalated to urgent.","severity":"critical","channel":"email","sourceModule":"billing","entityKind":"billing.recovery_case","entityPublicId":"recovery-001"}' "$base_url/api/notification/center")"
  notification_public_id="$(printf '%s' "$create_response" | extract_json_field "publicId")"
  transition_response="$(curl -fsS -X PATCH -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","status":"read"}' "$base_url/api/notification/center/$notification_public_id/status")"
  center_response="$(curl -fsS "$base_url/api/notification/center?tenant_slug=bootstrap-ops")"
  summary_response="$(curl -fsS "$base_url/api/notification/summary?tenant_slug=bootstrap-ops")"
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        tenant.slug || '|' ||
        (SELECT count(*) FROM notification.preferences AS preference WHERE preference.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM notification.notifications AS notification WHERE notification.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM notification.notifications AS notification WHERE notification.tenant_id = tenant.id AND notification.status = 'read') || '|' ||
        (SELECT count(*) FROM notification.notifications AS notification WHERE notification.tenant_id = tenant.id AND notification.severity = 'critical')
      FROM identity.tenants AS tenant
      WHERE tenant.slug = 'bootstrap-ops'
      LIMIT 1;
    ")"

  echo "[test] notification capabilities => $capabilities_response"
  echo "[test] notification preference => $preference_response"
  echo "[test] notification create => $create_response"
  echo "[test] notification transition => $transition_response"
  echo "[test] notification center => $center_response"
  echo "[test] notification summary => $summary_response"
  echo "[test] notification db summary => $db_summary"

  if [[ "$capabilities_response" != *'"key":"notification.center"'* || "$preference_response" != *'"emailEnabled":true'* || -z "$notification_public_id" || "$create_response" != *'"severity":"critical"'* || "$transition_response" != *'"status":"read"'* || "$center_response" != *'"pageInfo"'* || "$summary_response" != *'"total":1'* || "$summary_response" != *'"critical":1'* || "$db_summary" != 'bootstrap-ops|1|1|1|1' ]]; then
    echo "[test] notification runtime state did not expose the expected center cycle"
    exit 1
  fi
}

run_fiscal_runtime_smoke() {
  local base_url="http://localhost:${FISCAL_HTTP_PORT:-8102}"
  local capabilities_response
  local profile_response
  local retention_response
  local document_response
  local document_public_id
  local correction_response
  local cancel_response
  ...
  "${COMPOSE_CMD[@]}" up -d --build fiscal
  wait_for_http_ready "$base_url/health/ready"

  capabilities_response="$(curl -fsS "$base_url/api/fiscal/capabilities")"
  profile_response="$(curl -fsS -X PUT -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","taxRegime":"lucro_real","cnae":"6201-5/01","stateRegistration":"123456789","municipalRegistration":"998877","certificateMode":"a1","certificateLabel":"erp-local-cert","environmentMode":"homologation","actor":"fiscal@erp.local"}' "$base_url/api/fiscal/companies/company-001/profile")"
  retention_response="$(curl -fsS -X PUT -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","classification":"restricted","retentionDays":1825,"anonymizeAfterDays":3650,"source":"policy"}' "$base_url/api/fiscal/companies/company-001/retention-policies/documents")"
  document_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","companyPublicId":"company-001","documentKind":"nfe","seriesCode":"1","numberCode":"NF-1001","customerPublicId":"customer-001","amountCents":159900,"providerKey":"local","actor":"fiscal@erp.local"}' "$base_url/api/fiscal/documents")"
  document_public_id="$(printf '%s' "$document_response" | extract_json_field "publicId")"
  correction_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","correctionText":"Corrected tax nature and service code.","actor":"fiscal@erp.local"}' "$base_url/api/fiscal/documents/$document_public_id/correction-letter")"
  cancel_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","reason":"Correction required.","actor":"fiscal@erp.local"}' "$base_url/api/fiscal/documents/$document_public_id/cancel")"
  consent_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","companyPublicId":"company-001","subjectKind":"customer","subjectPublicId":"customer-001","purposeKey":"marketing.email","status":"granted","source":"crm","actor":"dpo@erp.local"}' "$base_url/api/fiscal/consents")"
  privacy_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"tenantSlug":"bootstrap-ops","companyPublicId":"company-001","requestType":"anonymization","subjectKind":"customer","subjectPublicId":"customer-001","requestedBy":"dpo@erp.local","consentReference":"consent-001"}' "$base_url/api/fiscal/privacy-requests")"
  audit_response="$(curl -fsS "$base_url/api/fiscal/audit-events?tenant_slug=bootstrap-ops")"
  summary_response="$(curl -fsS "$base_url/api/fiscal/compliance/summary?tenant_slug=bootstrap-ops")"
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        tenant.slug || '|' ||
        (SELECT count(*) FROM fiscal.company_profiles AS profile WHERE profile.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM fiscal.retention_policies AS policy WHERE policy.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM fiscal.documents AS document WHERE document.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM fiscal.document_events AS event WHERE event.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM fiscal.consents AS consent WHERE consent.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM fiscal.privacy_requests AS request WHERE request.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM fiscal.audit_events AS event WHERE event.tenant_id = tenant.id)
      FROM identity.tenants AS tenant
      WHERE tenant.slug = 'bootstrap-ops'
      LIMIT 1;
    ")"

  echo "[test] fiscal capabilities => $capabilities_response"
  echo "[test] fiscal profile => $profile_response"
  echo "[test] fiscal retention => $retention_response"
  echo "[test] fiscal document => $document_response"
  echo "[test] fiscal correction => $correction_response"
  echo "[test] fiscal cancel => $cancel_response"
  echo "[test] fiscal consent => $consent_response"
  echo "[test] fiscal privacy => $privacy_response"
  echo "[test] fiscal audit => $audit_response"
  echo "[test] fiscal summary => $summary_response"
  echo "[test] fiscal db summary => $db_summary"

  if [[ "$capabilities_response" != *'"capabilityKey":"fiscal.nfe"'* || "$capabilities_response" != *'"capabilityKey":"fiscal.certificate.a1"'* || "$profile_response" != *'"taxRegime":"lucro_real"'* || "$retention_response" != *'"classification":"restricted"'* || -z "$document_public_id" || "$document_response" != *'"documentKind":"nfe"'* || "$correction_response" != *'"status":"corrected"'* || "$cancel_response" != *'"status":"cancelled"'* || "$consent_response" != *'"status":"granted"'* || "$privacy_response" != *'"requestType":"anonymization"'* || "$audit_response" != *'"category":"company_profile"'* || "$summary_response" != *'"fiscalDocuments":1'* || "$summary_response" != *'"consents":1'* || "$summary_response" != *'"documentEvents":3'* || "$db_summary" != 'bootstrap-ops|1|1|1|3|1|1|6' ]]; then
    echo "[test] fiscal runtime state did not expose the expected compliance cycle"
    exit 1
  fi
}
...
run_platform_control_runtime_smoke() {
  ...
  local go_live_readiness_response
  ...
  local go_live_response
  local go_live_public_id
  local go_live_start_response
  local go_live_complete_response
  local go_live_rollouts_response
  local go_live_detail_response
  ...
  go_live_readiness_response="$(curl -fsS "$base_url/api/platform-control/tenants/bootstrap-ops/go-live/readiness")"
  ...
  go_live_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"requestedBy":"ops@bootstrap-ops.local","targetEnv":"production","waveKey":"wave-1","rollbackPlaybook":"docs/OPERACOES.md#rollback","adoptionTargetPct":80}' "$base_url/api/platform-control/tenants/bootstrap-ops/go-live/rollouts")"
  go_live_public_id="$(printf '%s' "$go_live_response" | extract_json_field "publicId")"
  go_live_start_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"summary":"Wave started."}' "$base_url/api/platform-control/tenants/bootstrap-ops/go-live/rollouts/$go_live_public_id/start")"
  go_live_complete_response="$(curl -fsS -X POST -H "Content-Type: application/json" -d '{"summary":"Wave completed."}' "$base_url/api/platform-control/tenants/bootstrap-ops/go-live/rollouts/$go_live_public_id/complete")"
  go_live_rollouts_response="$(curl -fsS "$base_url/api/platform-control/tenants/bootstrap-ops/go-live/rollouts")"
  go_live_detail_response="$(curl -fsS "$base_url/api/platform-control/tenants/bootstrap-ops/go-live/rollouts/$go_live_public_id")"
  ...
  db_summary="$("${COMPOSE_CMD[@]}" exec -T service-postgresql psql -U "$DB_USER" -d "$DB_NAME" -At -c "
      SELECT
        ...
        (SELECT count(*) FROM platform_control.go_live_rollouts AS rollout WHERE rollout.tenant_id = tenant.id) || '|' ||
        (SELECT count(*) FROM platform_control.go_live_rollout_events AS event JOIN platform_control.go_live_rollouts AS rollout ON rollout.id = event.rollout_id WHERE rollout.tenant_id = tenant.id)
      FROM identity.tenants AS tenant
      WHERE tenant.slug = 'bootstrap-ops'
      LIMIT 1;
    ")"
  ...
  echo "[test] platform-control go-live readiness => $go_live_readiness_response"
  ...
  echo "[test] platform-control go-live rollout => $go_live_response"
  echo "[test] platform-control go-live start => $go_live_start_response"
  echo "[test] platform-control go-live complete => $go_live_complete_response"
  echo "[test] platform-control go-live rollouts => $go_live_rollouts_response"
  echo "[test] platform-control go-live detail => $go_live_detail_response"
...
run_edge_runtime_smoke() {
  ...
  local integrations_overview_response
  local documents_overview_response
  local collections_overview_response
  local platform_reliability_overview_response
  local hardening_overview_response
  local core_operations_overview_response
  local relationship_overview_response
  local compliance_overview_response
  local go_live_overview_response
  local saas_overview_response
  local contracts_overview_response
  ...
  integrations_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/integrations-overview?tenantSlug=bootstrap-ops")"
  ...
  platform_reliability_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/platform-reliability?tenantSlug=bootstrap-ops")"
  hardening_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/hardening-overview?tenantSlug=bootstrap-ops")"
  core_operations_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/core-operations?tenantSlug=bootstrap-ops")"
  relationship_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/relationship-overview?tenantSlug=bootstrap-ops")"
  compliance_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/compliance-overview?tenantSlug=bootstrap-ops")"
  go_live_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/go-live-overview?tenantSlug=bootstrap-ops")"
  saas_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/saas-overview?tenantSlug=bootstrap-ops")"
  contracts_overview_response="$(curl -fsS -H "Authorization: Bearer $session_token" "$base_url/api/edge/ops/contracts-overview?tenantSlug=bootstrap-ops")"
  ...
  echo "[test] edge integrations overview => $integrations_overview_response"
  ...
  echo "[test] edge platform reliability overview => $platform_reliability_overview_response"
  echo "[test] edge hardening overview => $hardening_overview_response"
  echo "[test] edge core operations overview => $core_operations_overview_response"
  echo "[test] edge relationship overview => $relationship_overview_response"
  echo "[test] edge compliance overview => $compliance_overview_response"
  echo "[test] edge go-live overview => $go_live_overview_response"
  echo "[test] edge saas overview => $saas_overview_response"
  echo "[test] edge contracts overview => $contracts_overview_response"
  ...
  if [[ "$platform_reliability_overview_response" != *'"service":"edge"'* || "$platform_reliability_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$platform_reliability_overview_response" != *'"pendingWebhookEvents":0'* || "$platform_reliability_overview_response" != *'"deadLetterEvents":1'* || "$platform_reliability_overview_response" != *'"failedWorkflowExecutions":0'* || "$platform_reliability_overview_response" != *'"criticalRecoveryCases":0'* || "$platform_reliability_overview_response" != *'"failedPaymentAttempts":2'* || "$platform_reliability_overview_response" != *'"webhookForwardingRateBps":3333'* || "$platform_reliability_overview_response" != *'"workflowSuccessRateBps":10000'* || "$platform_reliability_overview_response" != *'"billingRecoveryRateBps":10000'* || "$platform_reliability_overview_response" != *'"openCriticalRisks":1'* || "$platform_reliability_overview_response" != *'"platformReliability"'* || "$hardening_overview_response" != *'"service":"edge"'* || "$hardening_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$hardening_overview_response" != *'"stableChecks":8'* || "$hardening_overview_response" != *'"attentionChecks":3'* || "$hardening_overview_response" != *'"criticalChecks":1'* || "$hardening_overview_response" != *'"deadLetterEvents":1'* || "$hardening_overview_response" != *'"failedPaymentAttempts":2'* || "$hardening_overview_response" != *'"criticalProviderGaps":4'* || "$hardening_overview_response" != *'"httpSpecs":16'* || "$hardening_overview_response" != *'"latestBenchmarkStatus":"stable"'* || "$hardening_overview_response" != *'"backupRestoreValidated":true'* || "$hardening_overview_response" != *'"hardeningReview"'* || "$hardening_overview_response" != *'"operationalRunbooks"'* ]]; then
    echo "[test] edge platform reliability cockpit did not aggregate the expected live payload"
    exit 1
  fi

  if [[ "$core_operations_overview_response" != *'"service":"edge"'* || "$core_operations_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$core_operations_overview_response" != *'"coreOperations"'* || "$relationship_overview_response" != *'"service":"edge"'* || "$relationship_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$relationship_overview_response" != *'"relationshipIntelligence"'* || "$relationship_overview_response" != *'"importsReady":true'* || "$compliance_overview_response" != *'"service":"edge"'* || "$compliance_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$compliance_overview_response" != *'"complianceControl"'* || "$go_live_overview_response" != *'"service":"edge"'* || "$go_live_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$go_live_overview_response" != *'"goLiveControl"'* || "$go_live_overview_response" != *'"releaseControls"'* || "$go_live_overview_response" != *'"acceptanceReady":true'* || "$go_live_overview_response" != *'"rollbackReady":true'* || "$saas_overview_response" != *'"service":"edge"'* || "$saas_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$saas_overview_response" != *'"saasControl"'* || "$contracts_overview_response" != *'"service":"edge"'* || "$contracts_overview_response" != *'"status":"stable"'* || "$contracts_overview_response" != *'"schemaRegistryReady":true'* || "$contracts_overview_response" != *'"apiPortalReady":true'* ]]; then
    echo "[test] edge expansion and go-live cockpits did not aggregate the expected live payload"
    exit 1
  fi
}
...
run_smoke() {
  ...
  export CATALOG_HTTP_PORT="${CATALOG_HTTP_PORT_SMOKE:-18097}"
  export PLATFORM_CONTROL_HTTP_PORT="${PLATFORM_CONTROL_HTTP_PORT_SMOKE:-18098}"
  export SUPPORT_HTTP_PORT="${SUPPORT_HTTP_PORT_SMOKE:-18099}"
