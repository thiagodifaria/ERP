...
run_edge_runtime_smoke() {
  ...
  if [[ "$engagement_overview_response" != *'"service":"edge"'* || "$engagement_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$engagement_overview_response" != *'"status":"stable"'* || "$engagement_overview_response" != *'"campaigns":3'* || "$engagement_overview_response" != *'"activeCampaigns":2'* || "$engagement_overview_response" != *'"templates":2'* || "$engagement_overview_response" != *'"deliveries":3'* || "$engagement_overview_response" != *'"deliveredDeliveries":3'* || "$engagement_overview_response" != *'"failedDeliveries":0'* || "$engagement_overview_response" != *'"convertedTouchpoints":1'* || "$engagement_overview_response" != *'"deliveryRate":1'* || "$engagement_overview_response" != *'"engagementOperations"'* ]]; then
    echo "[test] edge engagement cockpit did not aggregate the expected live payload"
    exit 1
  fi

  if [[ "$integrations_overview_response" != *'"service":"edge"'* || "$integrations_overview_response" != *'"tenantSlug":"bootstrap-ops"'* || "$integrations_overview_response" != *'"status":"attention"'* || "$integrations_overview_response" != *'"configuredProviders":1'* || "$integrations_overview_response" != *'"activeInboundProviders":1'* || "$integrations_overview_response" != *'"activeOutboundProviders":1'* || "$integrations_overview_response" != *'"inboundLeads":0'* || "$integrations_overview_response" != *'"workflowDispatches":1'* || "$integrations_overview_response" != *'"failedProviderEvents":0'* || "$integrations_overview_response" != *'"deadLetterEvents":1'* || "$integrations_overview_response" != *'"openProviderRisks":1'* || "$integrations_overview_response" != *'"integrationReadiness"'* ]]; then
