...
- touchpoint stream with thread and participant ownership for structured conversations
...
- touchpoints now persist `threadPublicId`, `participantKind` and `participantPublicId`, allowing the omnichannel stream to be read as structured conversations instead of isolated messages
