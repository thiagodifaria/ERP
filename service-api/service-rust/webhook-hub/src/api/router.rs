...
fn build_router_with_state(state: WebhookHubState) -> Router {
    Router::new()
        ...
        .route_layer(from_fn(security_middleware))
        .with_state(state)
}

async fn security_middleware(mut request: Request, next: Next) -> Response {
    ...
    if requires_correlation(request.method().as_str())
        && request
            ...
            .is_empty()
    {
        return security_error(StatusCode::BAD_REQUEST, "correlation_id_required", "Mutation requests require X-Correlation-Id.");
    }
    let Some(auth) = authenticate(&request) else {
        return security_error(StatusCode::UNAUTHORIZED, "unauthorized", "Bearer token is invalid or missing.");
    };
    ...
    if !authorize_openfga("webhook-hub", &request_method, &auth).await {
        return security_error(StatusCode::FORBIDDEN, "openfga_denied", "OpenFGA denied the request.");
    }
...
fn authenticate(request: &Request) -> Option<AuthContext> {
    ...
    if !internal_token.trim().is_empty() && token.as_bytes() == internal_token.as_bytes() {
