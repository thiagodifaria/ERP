...
def load_settings() -> Settings:
    return Settings(
        service_name=os.getenv("PROCUREMENT_SERVICE_NAME", "procurement"),
        http_port=int(os.getenv("PROCUREMENT_HTTP_PORT", "8105")),
        repository_driver=os.getenv("PROCUREMENT_REPOSITORY_DRIVER", "memory"),
        bootstrap_tenant_slug=os.getenv("PROCUREMENT_BOOTSTRAP_TENANT_SLUG", "bootstrap-ops"),
        postgres_host=os.getenv("PROCUREMENT_POSTGRES_HOST", "service-postgresql"),
        postgres_port=int(os.getenv("PROCUREMENT_POSTGRES_PORT", "5432")),
        ...
        postgres_ssl_mode=os.getenv("PROCUREMENT_POSTGRES_SSL_MODE", "disable"),
    )
